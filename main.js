// minimize the native query selector funciton of the document
var $ = function(sel){ return document.querySelector(sel); };

// ================
// QUESTION 1: Show me how to calculate the average price of all items.
// ================
// map through the items and collect the prices into an array; add the prices in the array and store the final amount in a variable called 'total'
var total = items.map(function(i){ return i.price; }).reduce(function(a, b){ return a + b; });
// ALTERNATE APPROACH: Declare a variable 'total'; for each item 'i' in 'items', add the items price to 'total'
// var total = 0;
...
var average = total / items.length, // use 'total' and the number of items to find the average
    ans = "The average price is $" + average.toFixed(2) + ".";
...
filteredItems.forEach(function(i){ // for each filtered item 'i'
    var itemName = document.createElement("p"); // create a node on the page with a tagname <p> and store it as a variable called 'itemName'
...
woodenItems.forEach(function(i){ // for each wooden item 'i'
    var itemName = document.createElement("p"); // create a node on the page with a tagname <p> and store it as a variable called 'itemName'
    itemName.textContent = "\"" + i.title + '\" is made of wood.'; // add the name of the wooden item 'i' inside the <p> tag 'itemName'
...
// ================
// Filter items with a property 'materials' that contains 8 or more strings into an array called 'fancyItems'
...
fancyItems.forEach(function(i){ // for each fancy item 'i'
    ...
    i.materials.forEach(function(m){ // for each fancy item 'i' with a property 'materials' called 'm'
        ...
        fancyItemMaterials.appendChild(material); // add the material <li> to the ordered list 'fancyItemMaterials'
    });
